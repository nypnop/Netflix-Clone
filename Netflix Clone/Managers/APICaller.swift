...
//    static let YT_API_KEY = "AIzaSyC25R9FVPQSH2btuPxYh7NDrj7WLERDkDU"
    static let YoutubeAPI_KEY = "AIzaSyB1ydvqUXguvvWy51zEfSKOWiJTmMLqo2s"
...
//                print(results)
    ...
    func getMoviesbyGenre(with genre: Int, completion: @escaping (Result<[Title], Error>) -> Void){
        guard let url = URL(string: "\(Constants.baseURL)/3/discover/movie?api_key=\(Constants.API_KEY)&include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=\(genre)") else {return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else {return}
            do {
                let results = try JSONDecoder().decode(TrendingTitlesResponse.self, from: data)
